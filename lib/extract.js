...
var Extract = function (opts) {
  ...
  function onsymlink () {
    ...
    try {
      for (var i = 0; i < data.length; i++) checksum += data.readUInt8(i)
      var check = self._header._check
      if (check !== undefined && check !== checksum) {
        throw new Error('Bad checksum "' + checksum + '", expected "' + check + '"')
      }
  ...
  function onname () {
    ...
    if (header.type === 'symlink') {
      self._parse(header._sizeStrike, onsymlink)
      return oncontinue()
    }
...
Extract.prototype._write = function (data, enc, cb) {
  ...
  if (data.length < strike) {
    ...
    if (s) {
      ...
      for (var i = 0; i < data.length; i++) this._checksum += data.readUInt8(i)
    } else {
      b.append(data)
      cb()
    }
